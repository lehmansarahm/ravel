...
CREATE OR REPLACE VIEW tenant_policy AS (
       SELECT DISTINCT fid, src as host1, dst as host2 FROM rm
       WHERE src IN (SELECT * FROM tenant_hosts)
       	     AND dst IN (SELECT * FROM tenant_hosts)
);
...
if (h1 in hosts) & (h2 in hosts):
    plpy.execute ("INSERT INTO rm values (" + str (fid)  + "," +str (h1)+ "," + str (h2) + ");")
